...
def test_get_type_by_name(api, checker, type_name):
    with allure.step(f'Запрос типа "{type_name}"'):
        ...
        with allure.step(f'Проверка, что в ответе приходит тип "{type_name}"'):
            checker.json_value(response, field_name='name', expected_value=type_name)
...
def test_get_type_by_id(api, checker, type_id, type_name):
...
def test_get_types_list(api, checker, limit, offset):
...
def test_404_for_invalid_type(api, checker):
